...
def get_address():
    ...
    with open(full_path('data.csv'), 'r', newline='', encoding='utf-8-sig') as f:
        rows = list(csv.DictReader(f))

    result = {}
    for csv_key, out_key in addr_fields:
        candidates = [
            r[csv_key].strip()
            for r in rows
            if (csv_key in r) and r[csv_key] and r[csv_key].strip()
        ]
        result[out_key] = choice(candidates) if candidates else ''

    if not result.get('country'):
        result['country'] = 'India' if result.get('state') else 'Unknown'
